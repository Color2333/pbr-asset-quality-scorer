...
Usage:
    python asset_quality_scorer/scripts/train_v2_regression.py --channel metallic
    python asset_quality_scorer/scripts/train_v2_regression.py --all   # sequential
"""
...
from quality_scorer.data_v2 import (
    CHANNEL_DEFECT_COLS,
    TensorCacheCLIPDataset,
    build_score_lookup,
)
from quality_scorer.metrics_v2 import eval_regression_epoch
...
def _save_checkpoint(out_dir, fname, model, optimizer, epoch, channel, args, metrics):
    state = dict(
        ...
        channel=channel,
        arch="convnext_base_regression_clip_v2",
...
def _ranking_loss(pred: torch.Tensor, target: torch.Tensor, margin: float) -> torch.Tensor:
    """Pairwise ranking loss: enforces pred_i > pred_j when target_i > target_j."""
    ...
    # Only penalise pairs where true scores differ
...
def train_channel(args: argparse.Namespace, channel: str) -> dict:
    ...
    out_dir = _resolve(args.output_root) / f"convnext_base_{channel}_regression_v2"
    out_dir.mkdir(parents=True, exist_ok=True)

    defect_cols = CHANNEL_DEFECT_COLS.get(channel, [])

    print(f"\n{'='*72}")
    print(f"Phase 2 regression scorer: {channel}")
    print(f"{'='*72}")
    ...
    print(f"  tail_oversample : {args.tail_oversample_factor}x  lo={args.tail_lo_score}" if args.tail_oversample_factor else "  tail_oversample : (none)")
    ...
    log = dict(channel=channel, arch="convnext_base_regression_clip_v2",
    ...
    for ei in range(args.epochs):
        ...
        for imgs, clips, scores, binaries, defects in tqdm(
            ...
            # Pairwise ranking loss (encourages correct ordering → SRCC)
    ...
    (out_dir / "summary.json").write_text(
        json.dumps(dict(channel=channel, best=best, last_log=log), indent=2)
    )
    return dict(channel=channel, best=best, output_dir=str(out_dir))
...
def parse_args() -> argparse.Namespace:
    ...
    # Per-channel oversample overrides: config["train"]["channel_oversample"][channel]
    channel_for_override = (args.channel if args.channel else None)
    _channel_ovr = t.get("channel_oversample", {})
    _ovr = _channel_ovr.get(channel_for_override, {}) if channel_for_override else {}
    ...
    _hi_default = t.get("hi_score", None)
    _hi_val = _ovr.get("hi_score", _hi_default)
    args.oversample_hi_score   = int(_hi_val) if _hi_val is not None else None
    _tail_val = _ovr.get("tail_oversample_factor", t.get("tail_oversample_factor", None))
    args.tail_oversample_factor = float(_tail_val) if _tail_val is not None else None
...
def main() -> int:
    ...
    print("\nDone.")
    for s in summaries:
        print(f"  {s['channel']}: {s['best']}  → {s['output_dir']}")
    return 0
