"""Dataset v2: tensor cache (224×224 memmap) + pre-computed CLIP features + defect labels."""
...
# Defect label columns available per channel in the manifest CSV
CHANNEL_DEFECT_COLS: dict[str, list[str]] = {
    "base_color": ["has_text_or_pattern", "base_color_fake_ao_or_glow"],
    "normal_map":  ["normal_abnormal_tint"],   # normal_is_flipped removed: only 0.5% positive (~190 samples)
...
def _load_manifest_defect_labels(
    ...
    """Returns {model_name: [label_0, label_1, ...]} for the requested columns."""
...
class TensorCacheCLIPDataset(Dataset):
    ...
    def __init__(
        ...
        bc = clip_data["features"]["base_color"].float()   # (N, 768)
        rn = clip_data["features"]["render"].float()       # (N, 768)
        self._clip_tensor: torch.Tensor = torch.cat([bc, rn], dim=1)  # (N, 1536)
    ...
    def __getitem__(self, idx: int):
        ...
        # binary auxiliary label
        binary = float(score <= self.invalid_max_score)

        # defect auxiliary labels
        if self.defect_cols:
            defect = torch.tensor(
    ...
    def get_sample_weights(
        ...
        # Oversample scores in [lo_score, hi_score] by mid_factor; others stay at 1.0.
        # lo_score=0   → include score=0 (for channels where it is rare, e.g. base_color)
        # hi_score=4   → exclude score=5 (for channels where it is already common, e.g. metallic)
        # tail_factor  → separate higher boost for s >= tail_lo_score (takes priority over mid)
        #                useful when score=4/5 are severely underrepresented (e.g. normal_map)
        if hi_score is None:
